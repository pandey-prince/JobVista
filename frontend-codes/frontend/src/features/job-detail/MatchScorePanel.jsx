...
const MatchScorePanel = ({ matchScore, matchLoading, onLoad, showForStudent }) => {
  ...
  return (
    <div className="rounded-lg border border-[#e4d8ff] bg-[#f7f4ff] p-5">
      ...
      {matchLoading ? (
        ...
        <div className="mt-4">
          <p className="text-3xl font-bold text-[#6A38C2]">{matchScore.score}%</p>
          ...
          {matchScore.tip && <p className="mt-3 text-sm text-gray-600">{matchScore.tip}</p>}
        </div>
      ) : (
