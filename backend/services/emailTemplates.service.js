const FRONTEND_URL = process.env.FRONTEND_URL || "https://job-vista-eta.vercel.app";
...
export const buildDigestEmail = ({ userName, alertName, jobs }) => {
  ...
  const subject = `JobVista: ${jobs.length} new job${jobs.length === 1 ? "" : "s"} for "${alertName}"`;

  const html = `
    <div style="font-family:sans-serif;max-width:560px;margin:0 auto;color:#111;">
      <h2 style="color:#6A38C2;margin-bottom:4px;">JobVista</h2>
  ...
  const text = `JobVista alert "${alertName}" matched ${jobs.length} jobs. Visit ${FRONTEND_URL}/jobs`;
...
export const buildWatchlistEmail = ({ userName, companyName, jobs }) => {
  const rows = jobs.slice(0, 10).map(jobRow).join("");
  const subject = `JobVista: ${jobs.length} new job${jobs.length === 1 ? "" : "s"} at ${companyName}`;
